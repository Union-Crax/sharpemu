...
{
    private static readonly Hyper5Logger Log = Hyper5Log.For("VMEM");
    ...
    {
        var configured = Environment.GetEnvironmentVariable("HYPER5_LAZY_RESERVE_PRIME_MB");
    ...
    {
        if (!string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_VMEM"), "1", StringComparison.Ordinal))
        {
            return;
        }
