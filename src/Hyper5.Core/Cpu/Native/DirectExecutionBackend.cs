...
using Hyper5.Core.Cpu.Native.Windows;
using Hyper5.Core.Loader;
using Hyper5.Core.Memory;
using Hyper5.HLE;
...
{
	private static readonly Hyper5Logger Log = Hyper5Log.For("Native");
...
#pragma warning restore CS0649
	...
	// HYPER5_LOG_THREAD_MODE=1 — GC thread-mode corruption investigation. Traces
	...
	private static readonly bool LogThreadMode =
		string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_THREAD_MODE"), "1", StringComparison.Ordinal);
	...
	public bool TryExecute(CpuContext context, ulong entryPoint, Generation generation, IReadOnlyDictionary<ulong, string> importStubs, IReadOnlyDictionary<string, ulong> runtimeSymbols, CpuExecutionOptions executionOptions, out OrbisGen2Result result)
	{
		...
		_logStrlenImports = string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_STRLEN"), "1", StringComparison.Ordinal);
		_logStrlenBursts = _logStrlenImports ||
			string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_STRLEN_BURSTS"), "1", StringComparison.Ordinal);
		_logGuestContext = string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_CONTEXT"), "1", StringComparison.Ordinal);
		_logGuestThreads = string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_GUEST_THREADS"), "1", StringComparison.Ordinal);
		_logUsleep = string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_USLEEP"), "1", StringComparison.Ordinal);
		_logBootstrap = string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_BOOTSTRAP"), "1", StringComparison.Ordinal);
		_logAllImports = string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_ALL_IMPORTS"), "1", StringComparison.Ordinal);
		_logImportFrames = string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_IMPORT_FRAMES"), "1", StringComparison.Ordinal);
		_logImportRecent = string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_IMPORT_RECENT"), "1", StringComparison.Ordinal);
		_logStackCheck = string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_STACK_CHK"), "1", StringComparison.Ordinal);
		_probeImportReturn = Environment.GetEnvironmentVariable("HYPER5_PROBE_IMPORT_RET");
		_importFilter = Environment.GetEnvironmentVariable("HYPER5_LOG_IMPORT_FILTER");
		_disableImportLoopGuard = string.Equals(
			Environment.GetEnvironmentVariable("HYPER5_DISABLE_IMPORT_LOOP_GUARD"),
			"1",
	...
	{
		if (nid == "1jfXLRVzisc" &&
			string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_USLEEP"), "1", StringComparison.Ordinal))
		{
			address = 0;
			return false;
		}
	...
	private bool PreferLleForLibcExport(string exportName)
	{
		...
		if (string.Equals(Environment.GetEnvironmentVariable("HYPER5_DISABLE_LLE_LIBC"), "1", StringComparison.Ordinal))
		{
			return false;
		}
		var value = Environment.GetEnvironmentVariable("HYPER5_LLE_LIBC_SAFE_ONLY");
		...
		if (string.Equals(Environment.GetEnvironmentVariable("HYPER5_LLE_LIBC_ALL"), "1", StringComparison.Ordinal))
		{
			return true;
		}
	...
	private unsafe bool ExecuteEntry(CpuContext context, ulong entryPoint, out OrbisGen2Result result)
	{
		...
		try
		{
			...
			*(ulong*)(ptr2 + num3) = num2;
			...
			if (string.Equals(Environment.GetEnvironmentVariable("HYPER5_SENTINEL_PROBE"), "1", StringComparison.Ordinal))
			{
	...
	{
		if (int.TryParse(Environment.GetEnvironmentVariable("HYPER5_STALL_WATCHDOG_SECONDS"), out var result))
		{
	...
	// HYPER5_PERIODIC_SNAPSHOT_SECONDS=N: dump the stall snapshot every N
	...
	{
		if (int.TryParse(Environment.GetEnvironmentVariable("HYPER5_PERIODIC_SNAPSHOT_SECONDS"), out var result))
		{
			return Math.Max(0, result);
		}
		return 0;
	...
	private void StartStallWatchdog()
	{
		...
		{
			IsBackground = true,
			Name = "Hyper5-GuestThreadDispatcher"
		};
		...
		_stallWatchdogThread = new Thread(new ThreadStart(delegate
		{
			while (!_stallWatchdogStop)
			{
					...
					Stopwatch.GetTimestamp() - lastPeriodicSnapshot >= periodicSnapshotTicks)
				{
					...
					// to stderr otherwise.
					var snapshotPath = Environment.GetEnvironmentVariable("HYPER5_PERIODIC_SNAPSHOT_FILE");
