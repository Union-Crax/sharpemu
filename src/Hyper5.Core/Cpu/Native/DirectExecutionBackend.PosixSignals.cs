...
public sealed unsafe partial class DirectExecutionBackend
{
	...
	{
		if (string.Equals(Environment.GetEnvironmentVariable("HYPER5_DISABLE_POSIX_SIGNALS"), "1", StringComparison.Ordinal))
		{
			Console.Error.WriteLine("[LOADER][WARN] POSIX signal exception bridge disabled by HYPER5_DISABLE_POSIX_SIGNALS=1; guest faults will not be recovered.");
			return;
		}
		...
		_posixRawRecoveryEnabled = !string.Equals(
			Environment.GetEnvironmentVariable("HYPER5_DISABLE_RAW_HANDLER"), "1", StringComparison.Ordinal);
		if (!_posixRawRecoveryEnabled)
		{
			Console.Error.WriteLine("[LOADER][INFO] Raw sentinel recovery disabled by HYPER5_DISABLE_RAW_HANDLER=1");
		}
	...
	private static bool TryHandlePosixFault(int signal, nint siginfo, nint ucontext)
	{
		...
		bool traceSignal = traceIndex > 0 && (traceIndex <= 16 || traceIndex % 1024 == 0 ||
			string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_POSIX_SIGNALS"), "1", StringComparison.Ordinal));
		if (traceSignal)
		{
