...
public sealed partial class DirectExecutionBackend
{
	...
	{
		if (!string.Equals(Environment.GetEnvironmentVariable("HYPER5_DISABLE_RAW_HANDLER"), "1", StringComparison.Ordinal))
		{
		...
		{
			Console.Error.WriteLine("[LOADER][INFO] Raw exception handler disabled by HYPER5_DISABLE_RAW_HANDLER=1");
		}
	...
	{
		if (!string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_DISASM"), "1", StringComparison.Ordinal))
		{
			return;
		}
		...
		var extraAddresses = Environment.GetEnvironmentVariable("HYPER5_LOG_DISASM_ADDRS");
	...
	{
		var targetList = ParseDiagnosticAddresses(Environment.GetEnvironmentVariable("HYPER5_LOG_REFSCAN_ADDRS"));
	...
	{
		var targetList = ParseDiagnosticAddresses(Environment.GetEnvironmentVariable("HYPER5_LOG_POINTER_WINDOWS"));
		...
		var windowSize = 0x80;
		var rawWindowSize = Environment.GetEnvironmentVariable("HYPER5_LOG_POINTER_WINDOW_SIZE");
	...
	{
		if (string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_LAZY_COMMIT"), "1", StringComparison.Ordinal))
		{
			return true;
		}
