...
public sealed partial class DirectExecutionBackend
{
		...
		try
		{
			...
			if (GuestThreadExecution.TryConsumeCurrentContextTransfer(out var transferTarget))
			{
				...
				*(ulong*)(argPackPtr + 96) = unchecked((ulong)transferStub);
				if (string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_FIBER"), "1", StringComparison.Ordinal))
				{
					Console.Error.WriteLine(
	...
	private static bool ShouldLogExpectedImportResults() =>
		string.Equals(
			Environment.GetEnvironmentVariable("HYPER5_LOG_EXPECTED_IMPORT_RESULTS"),
			"1",
	...
	{
		if (int.TryParse(Environment.GetEnvironmentVariable("HYPER5_IMPORT_LOOP_GUARD_SECONDS"), out var seconds))
		{
