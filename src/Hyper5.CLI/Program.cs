...
namespace Hyper5.CLI;

internal static partial class Program
{
    private static readonly Hyper5Logger Log = Hyper5Log.For("Hyper5");
    ...
    private const string MitigatedChildFlag = "--hyper5-mitigated-child";
    ...
    private const string MitigatedChildEnvironment = "HYPER5_MITIGATED_CHILD";
    ...
    private static bool CheckHostArchitecture()
    {
        ...
        Console.Error.WriteLine(
            ...
            "natively, so Hyper5 must run as an x86-64 process.");
        if (OperatingSystem.IsMacOS())
        {
            Console.Error.WriteLine(
    ...
    private static void PreloadMacVulkanLoader()
    {
        ...
        Console.Error.WriteLine(
            ...
            "Place a universal libMoltenVK.dylib (from the MoltenVK releases) next to Hyper5 " +
            "as libvulkan.1.dylib.");
    }

    private static int RunEmulator(string[] args, bool isMitigatedChild)
    {
        Console.Error.WriteLine($"[DEBUG] Hyper5 starting with {args.Length} args — https://github.com/union-crax/Hyper5");
        ...
        Hyper5Log.MinimumLevel = logLevel;
        ...
        using var runtime = Hyper5Runtime.CreateDefault(runtimeOptions);
        ...
        catch (Exception ex)
        {
            ...
            Log.Error("Hyper5 failed to run.", ex);
            return 3;
        }
        finally
        {
        ...
        Log.Info($"Hyper5 execution completed. Result={result} (0x{(int)result:X8})");
    ...
    private static bool TryRunMitigatedChild(string[] args, out int childExitCode)
    {
        ...
        if (string.Equals(Environment.GetEnvironmentVariable("HYPER5_DISABLE_MITIGATION_RELAUNCH"), "1", StringComparison.Ordinal))
        {
            return false;
        }
    ...
    {
        Log.Info("Usage: Hyper5 [--strict] [--trace-imports[=N]] [--cpu-engine=<native>] [--log-level=<level>] [--log-file[=<path>]] <path-to-eboot.bin>");
        Log.Info(@"Example: Hyper5 --cpu-engine=native --trace-imports=64 --log-level=debug --log-file ""E:\Games\...\eboot.bin""");
    }

    private static bool TryParseArguments(
        string[] args,
        out string ebootPath,
        out Hyper5RuntimeOptions runtimeOptions,
        ...
        out string? logFilePath)
    {
        if (args.Length == 0)
        {
            ...
            logLevel = Hyper5Log.MinimumLevel;
            logFilePath = null;
            return false;
        }
        ...
        for (var i = 0; i < args.Length; i++)
        {
            ...
            {
                if (i + 1 >= args.Length || !Hyper5Log.TryParseLevel(args[i + 1], out logLevel))
                {
                    ebootPath = string.Empty;
                    runtimeOptions = default;
                    logFilePath = null;
            ...
            if (argument.StartsWith(logLevelPrefix, StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (!Hyper5Log.TryParseLevel(valueText, out logLevel))
                {
                    ebootPath = string.Empty;
                    runtimeOptions = default;
                    return false;
        ...
        runtimeOptions = new Hyper5RuntimeOptions
        {
            CpuEngine = cpuEngine,
