...
public static class Hyper5Log
{
    private static readonly ConcurrentDictionary<string, Hyper5Logger> LoggerByCategory =
    ...
    private static IHyper5LogSink _sink = ResolveSinkFromEnvironment();

    /// <summary>
    /// Entries below this level are dropped. When a HYPER5_LOG_FILE sink is
    ...
    public static IHyper5LogSink Sink
    {
        get
        {
    ...
    public static void Configure(LogLevel? minimumLevel = null, IHyper5LogSink? sink = null)
    {
    ...
    public static Hyper5Logger For(string category)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(category);
        return LoggerByCategory.GetOrAdd(category, static key => new Hyper5Logger(key));
    }
    ...
    {
        var raw = Environment.GetEnvironmentVariable("HYPER5_LOG_LEVEL");
    ...
    private static bool ResolveColorEnabledFromEnvironment()
    {
        ...
        var raw = Environment.GetEnvironmentVariable("HYPER5_LOG_NO_COLOR");
        return !IsTrueLike(raw);
    }

    private static IHyper5LogSink ResolveSinkFromEnvironment()
    {
        var consoleSink = new ConsoleLogSink(
            useColors: ResolveColorEnabledFromEnvironment(),
            includeTimestamp: false);

        var logFilePath = Environment.GetEnvironmentVariable("HYPER5_LOG_FILE");
        if (!string.IsNullOrWhiteSpace(logFilePath))
        {
            ...
            catch (Exception ex)
            {
                ...
                Console.Error.WriteLine($"[HYPER5_LOG] Failed to open log file '{logFilePath}': {ex.Message}");
            }
        }
    ...
    /// </summary>
    private sealed class MinimumLevelFilterSink : IHyper5LogSink
    {
        private readonly IHyper5LogSink _inner;

        internal MinimumLevelFilterSink(IHyper5LogSink inner) => _inner = inner;
