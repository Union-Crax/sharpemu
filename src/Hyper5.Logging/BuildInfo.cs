...
{
    private const string ProjectUrl = "https://github.com/union-crax/Hyper5";
    private const string CanonicalRepository = "union-crax/Hyper5";
    ...
    static BuildInfo()
    {
        ...
        CommitSha = Normalize(metadata.GetValueOrDefault("Hyper5.BuildSha"));
        if (CommitSha is { Length: > 7 })
        {
            CommitSha = CommitSha[..7];
        }

        Branch = Normalize(metadata.GetValueOrDefault("Hyper5.BuildBranch"));
        Repository = Normalize(metadata.GetValueOrDefault("Hyper5.BuildRepository"));
        Configuration = Normalize(metadata.GetValueOrDefault("Hyper5.BuildConfiguration"));

        var serverUrl = Normalize(metadata.GetValueOrDefault("Hyper5.BuildServerUrl"));
        var runId = Normalize(metadata.GetValueOrDefault("Hyper5.BuildRunId"));
        ...
        var eventName = Normalize(metadata.GetValueOrDefault("Hyper5.BuildEventName"));
        var gitRef = Normalize(metadata.GetValueOrDefault("Hyper5.BuildRef"));
    ...
    /// <code>
    /// Hyper5 UNOFFICIAL f11ac59 — https://github.com/union-crax/Hyper5
    ///
    /// Built from branch "main" of "union-crax/Hyper5" by GitHub Actions workflow run https://github.com/union-crax/Hyper5/actions/runs/123.
    /// </code>
    ...
    public static string Banner
    {
        get
        {
            ...
            var header = $"Hyper5 {version} — {ProjectUrl}";
    ...
    private static Dictionary<string, string> ReadMetadata()
    {
        ...
        foreach (var assembly in assemblies)
        {
            ...
            {
                if (attribute.Key.StartsWith("Hyper5.Build", StringComparison.Ordinal) &&
