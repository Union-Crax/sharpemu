...
/// <summary>
/// Launches the Hyper5 CLI as a child process with the same CET/CFG mitigation
...
/// HYPER5_DISABLE_MITIGATION_RELAUNCH so output is not lost to a detached
...
internal sealed class EmulatorProcess : IDisposable
{
    ...
    private void StartWindows(string exePath, IReadOnlyList<string> arguments, string? workingDirectory)
    {
        ...
        Environment.SetEnvironmentVariable("HYPER5_DISABLE_MITIGATION_RELAUNCH", "1");
    ...
    private void StartReaderThread(nint readHandle, bool isError)
    {
        ...
        {
            IsBackground = true,
            Name = isError ? "Hyper5 stderr reader" : "Hyper5 stdout reader",
        };
        thread.Start();
    }
