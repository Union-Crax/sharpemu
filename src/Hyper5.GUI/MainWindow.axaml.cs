...
public partial class MainWindow : Window
{
    ...
    private const double MinimumConsoleHeight = 120;
    private const double DefaultInspectorWidth = 320;
    private const double MinimumInspectorWidth = 180;
    private const double MaximumInspectorWidth = 720;
    ...
    private GameEntry? _lastInspectorGame;
    private double _consoleHeight = DefaultConsoleHeight;
    ...
    private ControllerPromptStyle _controllerPromptStyle;

    private enum ControllerPromptStyle
    {
        None,
        PlayStation,
        Xbox,
    ...
    public MainWindow()
    {
        ...
        // The console row is collapsed until the user asks for it. Once open,
        // the GridSplitter owns its pixel height and can resize it freely.
        ...
        MaximizeRestoreButton.Click += (_, _) => ToggleMaximized();
        CloseButton.Click += (_, _) => Close();
        GameList.SelectionChanged += (_, _) => OnGameSelectionChanged();
        GameList.AddHandler(PointerPressedEvent, OnGameListPointerPressed, RoutingStrategies.Tunnel);
        GameList.DoubleTapped += (_, _) => LaunchSelected();
        SearchBox.TextChanged += (_, _) => RefreshVisibleGames();
        SortBox.SelectionChanged += (_, _) => RefreshVisibleGames();
        GridViewButton.Click += (_, _) => SetLibraryView(true);
        ListViewButton.Click += (_, _) => SetLibraryView(false);
        ...
        OpenGameFolderButton.Click += (_, _) => OpenSelectedGameFolder();
        ...
        ConsoleToggle.IsCheckedChanged += (_, _) => SetConsoleVisible(ConsoleToggle.IsChecked == true);
        InspectorToggleButton.Click += (_, _) => SetInspectorVisible(!_isInspectorVisible);
        InspectorCloseButton.Click += (_, _) => SetInspectorVisible(false);
        ...
        ControllerButtonsToggle.IsCheckedChanged += (_, _) =>
        {
            _settings.ShowControllerButtonIcons = ControllerButtonsToggle.IsChecked == true;
            UpdateControllerButtonPresentation();
        };
        ...
        EnvBthidToggle.IsCheckedChanged += (_, _) =>
            SetEnvironmentToggle("HYPER5_BTHID_UNAVAILABLE", EnvBthidToggle.IsChecked == true);
        EnvLoopGuardToggle.IsCheckedChanged += (_, _) =>
            SetEnvironmentToggle("HYPER5_DISABLE_IMPORT_LOOP_GUARD", EnvLoopGuardToggle.IsChecked == true);
        EnvVkValidationToggle.IsCheckedChanged += (_, _) =>
            SetEnvironmentToggle("HYPER5_VK_VALIDATION", EnvVkValidationToggle.IsChecked == true);
        EnvDumpSpirvToggle.IsCheckedChanged += (_, _) =>
            SetEnvironmentToggle("HYPER5_DUMP_SPIRV", EnvDumpSpirvToggle.IsChecked == true);
        EnvLogDirectMemoryToggle.IsCheckedChanged += (_, _) =>
            SetEnvironmentToggle("HYPER5_LOG_DIRECT_MEMORY", EnvLogDirectMemoryToggle.IsChecked == true);
        EnvLogNpToggle.IsCheckedChanged += (_, _) =>
            SetEnvironmentToggle("HYPER5_LOG_NP", EnvLogNpToggle.IsChecked == true);
        ...
        GithubButton.Click += (_, _) =>
        {
            ...
            {
                FileName = "https://github.com/union-crax/Hyper5",
                UseShellExecute = true
            });
        };

        OriginalRepoButton.Click += (_, _) =>
        {
            Process.Start(new ProcessStartInfo
            {
                FileName = "https://github.com/par274/hyper5",
                UseShellExecute = true
            });
        };
    ...
    private void SetActivePage(int index)
    {
        ...
        _ = UpdateBackdropAsync(index == 0 ? GameList.SelectedItem as GameEntry : null);
        ApplyInspectorVisibility();
        SetConsoleVisible(ConsoleToggle.IsChecked == true);

        // Keep the workspace heading aligned with the persistent navigation
        // rail so a page switch always has an immediate, visible cue.
        PageEyebrow.Text = index == 0 ? "LIBRARY" : "OPTIONS";
        PageTitle.Text = index == 0 ? "Library" : "Settings";
        PageSubtitle.Text = index == 0
            ? "Choose a title, review its details, and launch when ready."
            : "Tune emulation, diagnostics, and launcher behavior.";
    }

    /// <summary>
    /// Shows the game inspector when it is useful, while keeping its state
    /// independent from page switches. Choosing a title brings the inspector
    /// back; a close only hides it until the next title click.
    /// </summary>
    private void OnGameSelectionChanged()
    {
        ...
        if (!ReferenceEquals(_lastInspectorGame, selectedGame))
        {
            _lastInspectorGame = selectedGame;
    ...
    /// <summary>
    /// Re-selecting the already selected title does not raise ListBox's
    /// SelectionChanged event. Treat a pointer click on a title as an explicit
    /// request to open its inspector so closing the inspector never makes it
    /// appear permanently disconnected from the library.
    /// </summary>
    private void OnGameListPointerPressed(object? sender, PointerPressedEventArgs e)
    {
        ...
        _lastInspectorGame = game;
        _isInspectorVisible = true;

        // If the title was already selected there may be no SelectionChanged
        // callback to refresh the inspector, so do that part directly.
    ...
    {
        _isInspectorVisible = visible && GameList.SelectedItem is GameEntry;
        ApplyInspectorVisibility();
    }

    private void ApplyInspectorVisibility()
    {
        var hasSelection = GameList.SelectedItem is GameEntry;
        var visible = _isInspectorVisible && hasSelection && _activePageIndex == 0;

        var splitterColumn = WorkspaceGrid.ColumnDefinitions[2];
        var inspectorColumn = WorkspaceGrid.ColumnDefinitions[3];
        if (!visible)
        {
            RememberInspectorWidth();
        }

        splitterColumn.Width = visible
            ? new GridLength(4, GridUnitType.Pixel)
            : new GridLength(0, GridUnitType.Pixel);
        inspectorColumn.Width = visible
            ? new GridLength(_inspectorWidth, GridUnitType.Pixel)
            : new GridLength(0, GridUnitType.Pixel);

        SelectedGamePanel.IsVisible = visible;
        InspectorSplitter.IsVisible = visible;
        InspectorToggleButton.IsChecked = visible;
        InspectorToggleButton.IsVisible = hasSelection && _activePageIndex == 0;
    }

    private void RememberInspectorWidth()
    {
        if (!SelectedGamePanel.IsVisible)
        {
            return;
        }

        var actualWidth = WorkspaceGrid.ColumnDefinitions[3].ActualWidth;
        if (actualWidth > 1 && double.IsFinite(actualWidth))
        {
            _inspectorWidth = Math.Clamp(actualWidth, MinimumInspectorWidth, MaximumInspectorWidth);
            _settings.InspectorWidth = _inspectorWidth;
        }
    }

    private static double NormalizeInspectorWidth(double width)
    {
    ...
    private static string StripLeadingCommandGlyph(string text)
    {
        return text.TrimStart(' ', '+', '＋', '≡', '▶', '■').TrimStart();
    }

    private void SetConsoleVisible(bool requestedVisible)
    {
        var visible = requestedVisible && _consoleWindow is null && _activePageIndex == 0;
        var consoleRow = WorkspaceGrid.RowDefinitions[2];
        if (!visible && consoleRow.ActualHeight >= MinimumConsoleHeight)
        {
            _consoleHeight = consoleRow.ActualHeight;
        }

        consoleRow.Height = visible
            ? new GridLength(Math.Max(MinimumConsoleHeight, _consoleHeight), GridUnitType.Pixel)
            : new GridLength(0);
        ConsolePanel.IsVisible = visible;
        ConsoleSplitter.IsVisible = visible;
    }

    private void SetLibraryView(bool grid)
    {
        _isGridView = grid;
        GridViewButton.IsChecked = grid;
        ListViewButton.IsChecked = !grid;

        var panelKey = grid ? "GridItemsPanel" : "ListItemsPanel";
        var templateKey = grid ? "GridGameTemplate" : "ListGameTemplate";
        GameList.ItemsPanel = (ITemplate<Panel?>)Resources[panelKey]!;
        GameList.ItemTemplate = (IDataTemplate)Resources[templateKey]!;
        GameList.Classes.Set("tileGrid", grid);
        GameList.Classes.Set("gameList", !grid);
    }
    ...
    private void PollGamepad()
    {
        ...
        if ((pressed & HostGamepadButtons.Circle) != 0)
        {
            ...
            else if (ConsoleToggle.IsChecked == true)
            {
    ...
    private void SetControllerPromptStyle(ControllerPromptStyle style)
    {
        if (_controllerPromptStyle == style)
        {
            return;
        }

        _controllerPromptStyle = style;
        UpdateControllerButtonPresentation();
    }
    ...
    {
        // Tile footprint: 148 content + 14 item padding + 10 item margin.
        const double TileOuterWidth = 174;
    ...
    public async Task InitializeStartupAsync()
    {
        ...
        Title = $"Hyper5 {display}";
        ToolTip.SetTip(VersionText, BuildInfo.Banner);

        _settings = GuiSettings.Load();
        _inspectorWidth = NormalizeInspectorWidth(_settings.InspectorWidth);
    ...
    private void ApplyLocalization()
    {
        ...
        ToolTip.SetTip(LibraryTabButton, $"{loc.Get("Page.Library")} · L1");
        ToolTip.SetTip(OptionsTabButton, $"{loc.Get("Page.Options")} · R1 / Options");
        ToolTip.SetTip(ConsoleToggle, loc.Get("Console.Title"));
        ToolTip.SetTip(InspectorToggleButton, "Game details");

        SearchBox.Watermark = loc.Get("Library.SearchWatermark");
        AddFolderButtonLabel.Text = StripLeadingCommandGlyph(loc.Get("Library.AddFolder"));
        ...
        ControllerButtonsLabel.Text = loc.Get("Options.ControllerButtons.Label");
        ControllerButtonsDesc.Text = loc.Get("Options.ControllerButtons.Desc");
        ...
        foreach (var toggle in new[] { StrictToggle, LogToFileToggle, OverrideLogFileToggle, ControllerButtonsToggle, TitleMusicToggle, DiscordToggle })
        {
        ...
        ConsoleToggleLabel.Text = StripLeadingCommandGlyph(loc.Get("Launch.Console"));
        LaunchButtonLabel.Text = StripLeadingCommandGlyph(loc.Get("Launch.Launch"));
        StopButtonLabel.Text = StripLeadingCommandGlyph(loc.Get("Launch.Stop"));
    ...
    private void ApplyFullScreenLayout(bool fullScreen)
    {
        ...
        ExtendClientAreaToDecorationsHint = !fullScreen;
        RootLayout.RowDefinitions[0].Height = new GridLength(fullScreen ? 0 : 32);
        RootLayout.RowDefinitions[3].Height = new GridLength(fullScreen ? 0 : 26);
    ...
    {
        // WindowState can change while InitializeComponent is still creating
        // the visual tree, before the named caption controls are available.
        ...
        MaximizeRestoreGlyph.Data = (Geometry)Resources[maximized ? "FluentSquareMultiple" : "FluentSquare"]!;
        ToolTip.SetTip(MaximizeRestoreButton, maximized ? "Restore down" : "Maximize");
    }
    ...
    private void ApplySettingsToControls()
    {
        ...
        ControllerButtonsToggle.IsChecked = _settings.ShowControllerButtonIcons;
        TitleMusicToggle.IsChecked = _settings.PlayTitleMusic;
        DiscordToggle.IsChecked = _settings.DiscordRichPresence;
        EnvBthidToggle.IsChecked = _settings.EnvironmentToggles.Contains("HYPER5_BTHID_UNAVAILABLE");
        EnvLoopGuardToggle.IsChecked = _settings.EnvironmentToggles.Contains("HYPER5_DISABLE_IMPORT_LOOP_GUARD");
        EnvVkValidationToggle.IsChecked = _settings.EnvironmentToggles.Contains("HYPER5_VK_VALIDATION");
        EnvDumpSpirvToggle.IsChecked = _settings.EnvironmentToggles.Contains("HYPER5_DUMP_SPIRV");
        EnvLogDirectMemoryToggle.IsChecked = _settings.EnvironmentToggles.Contains("HYPER5_LOG_DIRECT_MEMORY");
        EnvLogNpToggle.IsChecked = _settings.EnvironmentToggles.Contains("HYPER5_LOG_NP");
        UpdateLogFilePathText();
        UpdateControllerButtonPresentation();
    }

    private void UpdateControllerButtonPresentation()
    {
        var showControllerButtons = _settings.ShowControllerButtonIcons;
        var showPlayStation = showControllerButtons && _controllerPromptStyle == ControllerPromptStyle.PlayStation;
        var showXbox = showControllerButtons && _controllerPromptStyle == ControllerPromptStyle.Xbox;
        var showPlatformPrompt = showPlayStation || showXbox;

        LaunchControllerGlyph.IsVisible = showPlatformPrompt;
        StopControllerGlyph.IsVisible = showPlatformPrompt;
        ConsoleControllerGlyph.IsVisible = showPlatformPrompt;
        InspectorControllerGlyph.IsVisible = showPlatformPrompt;

        LaunchPlayStationGlyph.IsVisible = showPlayStation;
        StopPlayStationGlyph.IsVisible = showPlayStation;
        ConsolePlayStationGlyph.IsVisible = showPlayStation;
        InspectorPlayStationGlyph.IsVisible = showPlayStation;
        LaunchXboxGlyph.IsVisible = showXbox;
        StopXboxGlyph.IsVisible = showXbox;
        ConsoleXboxGlyph.IsVisible = showXbox;
        InspectorXboxGlyph.IsVisible = showXbox;

        LaunchStandardGlyph.IsVisible = !showPlatformPrompt;
        StopStandardGlyph.IsVisible = !showPlatformPrompt;
        ConsoleStandardGlyph.IsVisible = !showPlatformPrompt;
        InspectorStandardGlyph.IsVisible = !showPlatformPrompt;

        CtxLaunch.Header = Localization.Instance.Get("Library.Context.Launch");
    }
    ...
    {
        var exeName = OperatingSystem.IsWindows() ? "Hyper5.exe" : "Hyper5";
        ...
        if (Environment.ProcessPath is { } selfPath &&
            Path.GetFileNameWithoutExtension(selfPath).Equals("Hyper5", StringComparison.OrdinalIgnoreCase))
        {
    ...
    private void RefreshVisibleGames()
    {
        ...
        var inspectorWasVisible = _isInspectorVisible;
        ...
        IEnumerable<GameEntry> orderedGames = SortBox.SelectedIndex switch
        {
            1 => filteredGames.OrderBy(game => game.TitleId ?? string.Empty, StringComparer.OrdinalIgnoreCase)
                              .ThenBy(game => game.Name, StringComparer.OrdinalIgnoreCase),
            2 => filteredGames.OrderByDescending(game => game.SizeBytes)
                              .ThenBy(game => game.Name, StringComparer.OrdinalIgnoreCase),
            _ => filteredGames.OrderBy(game => game.Name, StringComparer.OrdinalIgnoreCase),
        };
        ...
        _lastInspectorGame = GameList.SelectedItem as GameEntry;
        _isInspectorVisible = inspectorWasVisible && _lastInspectorGame is not null;
        EmptyState.IsVisible = _visibleGames.Count == 0;
        LibraryCountText.Text = _visibleGames.Count.ToString();
        UpdateEmptyStateTexts();
