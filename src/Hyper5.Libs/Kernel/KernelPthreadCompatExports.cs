...
public static class KernelPthreadCompatExports
{
    ...
    private static readonly bool _tracePthreads =
        string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_PTHREADS"), "1", StringComparison.Ordinal);
    private static readonly bool _tracePthreadConds =
        _tracePthreads ||
        string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_PTHREAD_CONDS"), "1", StringComparison.Ordinal);
    private static readonly HashSet<ulong>? _tracePthreadMutexFilter = ParseTraceAddressFilter(
        Environment.GetEnvironmentVariable("HYPER5_LOG_PTHREAD_MUTEX_FILTER"));
    private static readonly bool _enableMutexLockBlocking =
        string.Equals(Environment.GetEnvironmentVariable("HYPER5_MUTEX_LOCK_BLOCKING"), "1", StringComparison.Ordinal);
    ...
    {
        if (int.TryParse(Environment.GetEnvironmentVariable("HYPER5_PTHREAD_COND_SPURIOUS_WAKE_MS"), out var milliseconds))
        {
