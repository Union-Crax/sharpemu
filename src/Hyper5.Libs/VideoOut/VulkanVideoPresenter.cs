...
using Hyper5.HLE;
using Hyper5.Libs.Agc;
using Silk.NET.Input;
using Hyper5.Libs.Gpu;
using Hyper5.ShaderCompiler;
using Hyper5.ShaderCompiler.Vulkan;
...
namespace Hyper5.Libs.VideoOut;
...
internal static unsafe class VulkanVideoPresenter
{
    ...
    {
        var mode = Environment.GetEnvironmentVariable("HYPER5_TRACE_GUEST_IMAGES");
    ...
    private static bool ShouldTraceGuestImageSubmissionsForDiagnostics()
    {
        return string.Equals(
            Environment.GetEnvironmentVariable("HYPER5_TRACE_GUEST_IMAGES"),
            "1",
        ...
        try
        {
                    ...
                    vulkan, "vkGetInstanceProcAddr", out var procAddr))
            {
                Console.Error.WriteLine(
                    ...
                    "next to Hyper5 as libvulkan.1.dylib.");
                return;
            }
    ...
    /// Opt back into native Wayland with HYPER5_ENABLE_WAYLAND=1.
    /// </summary>
    private static unsafe void PreferX11OnLinuxWayland()
    {
        if (!OperatingSystem.IsLinux() ||
            string.Equals(
                Environment.GetEnvironmentVariable("HYPER5_ENABLE_WAYLAND"),
                "1",
                StringComparison.Ordinal))
        {
            return;
        ...
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("DISPLAY")))
        {
            Console.Error.WriteLine(
                ...
                "XWayland, or run natively with HYPER5_ENABLE_WAYLAND=1.");
            return;
        }
        ...
        try
        {
            ...
            Console.Error.WriteLine(
                ...
                "backend (set HYPER5_ENABLE_WAYLAND=1 to force native Wayland).");
        }
        catch (Exception exception)
        {
            Console.Error.WriteLine(
    ...
    private sealed class Presenter : IDisposable
    {
        ...
        private static readonly bool _performanceHudEnabled =
            !string.Equals(
                Environment.GetEnvironmentVariable("HYPER5_PERF_HUD"),
                "0",
                StringComparison.Ordinal);
        private Vk _vk = null!;
        ...
        private static readonly bool _dumpTextures = string.Equals(
            Environment.GetEnvironmentVariable("HYPER5_DUMP_TEXTURES"),
            "1",
            StringComparison.Ordinal);
        ...
        {
            public string DebugName = "Hyper5 translated";
        ...
        {
            var value = Environment.GetEnvironmentVariable("HYPER5_RENDERDOC_WAIT");
        ...
        private static string BuildComputeDebugName(VulkanComputeGuestDispatch dispatch)
        {
            ...
            return storage is null
                ? $"Hyper5 compute cs=0x{dispatch.ShaderAddress:X16} " +
        ...
        private static string GuestImageDebugName(GuestRenderTarget target, Format format) =>
            $"Hyper5 guest 0x{target.Address:X16} {target.Width}x{target.Height} " +
            $"fmt{target.Format}/{format}";

        private static string TextureDebugName(GuestDrawTexture texture, Format format) =>
            $"Hyper5 texture 0x{texture.Address:X16} {texture.Width}x{texture.Height} " +
            $"fmt{texture.Format}/{format}";

        private void CreateInstance()
        {
            var applicationName = (byte*)SilkMarshal.StringToPtr("Hyper5");
            var enableValidation = Environment.GetEnvironmentVariable("HYPER5_VK_VALIDATION") == "1";
            ...
            try
            {
                ...
                {
                    Console.Error.WriteLine("[LOADER][WARN] HYPER5_VK_VALIDATION=1 but VK_LAYER_KHRONOS_validation not found (Vulkan SDK installed?).");
                }
                ...
                try
                {
                    ...
                    if (validationLayerName is not null && _vk.TryGetInstanceExtension(_instance, out ExtDebugUtils debugUtils))
                    {
                        ...
                        Console.Error.WriteLine("[LOADER][INFO] Vulkan Validation Layers active (HYPER5_VK_VALIDATION=1).");
                    }
                }
                finally
                {
        ...
        private void SelectPhysicalDevice()
        {
            ...
            // the first hit. HYPER5_VK_DEVICE=<substring> pins an adapter by name.
            var deviceOverride = Environment.GetEnvironmentVariable("HYPER5_VK_DEVICE");
            ...
            try
            {
                ...
                fixed (VertexInputAttributeDescription* vertexAttributePointerBase = vertexAttributeDescriptions)
                {
                    ...
                    SetDebugName(
                        ObjectType.Pipeline,
                        pipeline.Handle,
                        $"Hyper5 graphics ps={fragmentSpirv.Length}b attrs={resources.Textures.Length}");
                }
            }
            finally
            {
            ...
            byte[] computeSpirv)
        {
            ...
            try
            {
                ...
                SetDebugName(
                    ObjectType.Pipeline,
                    pipeline.Handle,
                    $"Hyper5 compute cs={computeSpirv.Length}b");
        ...
        private TextureResource CreateStorageScratchResource(GuestDrawTexture texture)
        {
            ...
            SetDebugName(ObjectType.Image, image.Handle, $"Hyper5 scratch storage {width}x{height} {vkFormat}");
            SetDebugName(ObjectType.ImageView, view.Handle, $"Hyper5 scratch storage {width}x{height} {vkFormat} view");
            ...
            GuestMemoryBuffer guestBuffer)
        {
            ...
            SetDebugName(
                ObjectType.Buffer,
                buffer.Handle,
                $"Hyper5 global 0x{guestBuffer.BaseAddress:X16} {guestBuffer.Data.Length}b");
            ...
            GuestVertexBuffer guestBuffer)
        {
            ...
            SetDebugName(
                ObjectType.Buffer,
                buffer.Handle,
                $"Hyper5 vertex loc{guestBuffer.Location} " +
        ...
        private static readonly float ViewportDebugEpsilon = float.TryParse(
            Environment.GetEnvironmentVariable("HYPER5_VIEWPORT_EPSILON"),
        ...
        private void ExecuteComputeDispatch(VulkanComputeGuestDispatch work)
        {
            ...
            if (AddressListContains("HYPER5_SKIP_COMPUTE_CS", work.ShaderAddress))
            {
                TraceVulkanShader(
        ...
        private void ExecuteOffscreenDraw(VulkanOffscreenGuestDraw work)
        {
            ...
            try
            {
                ...
                resources.DebugName =
                    $"Hyper5 offscreen mrt={targets.Length} " +
                ...
                foreach (var target in targets)
                {
                    var traceSmallWrites =
                        Environment.GetEnvironmentVariable("HYPER5_TRACE_GUEST_WRITES") == "small" &&
            ...
            uint dstSelect = 0xFAC)
        {
            ...
            SetDebugName(
                ObjectType.ImageView,
                view.Handle,
                $"Hyper5 guest 0x{resource.Address:X16} alias {format} mip{mipLevel}+{levelCount}");
            TraceVulkanShader(
        ...
        {
            var directory =
                Environment.GetEnvironmentVariable("HYPER5_GUEST_IMAGE_DUMP_DIR");
            if (string.IsNullOrWhiteSpace(directory))
            {
                return;
            }
        ...
        {
            BeginDebugLabel(_commandBuffer, "Hyper5 swapchain draw");
            RecordTextureUploads(resources, PipelineStageFlags.FragmentShaderBit);
            RecordStorageImagesForWrite(resources, PipelineStageFlags.FragmentShaderBit);
            RecordTranslatedGraphicsPass(
                resources,
        ...
        private static bool ShouldTraceGuestImageAddressForDiagnostics(ulong address)
        {
            return AddressListContains(
                "HYPER5_TRACE_GUEST_IMAGE_ADDRS",
                address);
        }
        ...
        private static bool ShouldTraceVulkanResources() =>
            string.Equals(
                Environment.GetEnvironmentVariable("HYPER5_LOG_VK_RESOURCES"),
                "1",
                StringComparison.Ordinal);
            ...
            GuestImageResource source)
        {
            ...
            BeginDebugLabel(
                _commandBuffer,
                $"Hyper5 present image 0x{source.Address:X16}");
        ...
        {
            if (!string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_AGC"), "1", StringComparison.Ordinal) &&
                !string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_AGC_SHADER"), "1", StringComparison.Ordinal))
            {
                return;
            }
