...
public static class VideoOutExports
{
    ...
    private static string _windowTitleBase = "Hyper5 VideoOut";
    private static string _windowTitle = "Hyper5 VideoOut";
    private static readonly bool _logFrameRate = string.Equals(
        Environment.GetEnvironmentVariable("HYPER5_LOG_VIDEOOUT_FPS"),
        "1",
        StringComparison.Ordinal);
    private static readonly bool _logVideoOutSync = string.Equals(
        Environment.GetEnvironmentVariable("HYPER5_LOG_VIDEOOUT_SYNC"),
        "1",
        StringComparison.Ordinal);
    private static readonly bool _dumpVideoOut = string.Equals(
        Environment.GetEnvironmentVariable("HYPER5_DUMP_VIDEOOUT"),
        "1",
        StringComparison.Ordinal);
    ...
    public static void ConfigureApplicationInfo(string? title, string? titleId, string? version, string? emulatorCommitSha)
    {
        ...
        lock (_stateGate)
        {
            _windowTitleBase = $"Hyper5{commitSuffix} - {application}{versionSuffix}{hardwareSuffix}";
    ...
    private static string GetLogsDirectory()
    {
        ...
        {
            if (File.Exists(Path.Combine(current.FullName, "Hyper5.slnx")))
            {
    ...
    {
        if (!string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_VIDEOOUT"), "1", StringComparison.Ordinal))
        {
            return;
        }
