...
public static class AgcExports
{
    ...
    private static readonly bool _traceAgc = string.Equals(
        Environment.GetEnvironmentVariable("HYPER5_LOG_AGC"),
        "1",
        StringComparison.Ordinal);
    private static readonly bool _traceAgcShader =
        _traceAgc ||
        string.Equals(
            Environment.GetEnvironmentVariable("HYPER5_LOG_AGC_SHADER"),
            "1",
            StringComparison.Ordinal);
    private static readonly bool _traceTextureHashes = string.Equals(
        Environment.GetEnvironmentVariable("HYPER5_TRACE_TEXTURE_HASHES"),
        "1",
        StringComparison.Ordinal);
    ...
    public static int DriverSubmitMultiDcbs(CpuContext ctx)
    {
        ...
        var tracePackets = string.Equals(
            Environment.GetEnvironmentVariable("HYPER5_LOG_AGC"), "1", StringComparison.Ordinal);
    ...
    private static readonly bool _transparentFillClearEnabled = !string.Equals(
        Environment.GetEnvironmentVariable("HYPER5_DISABLE_TRANSPARENT_FILL_CLEAR"),
        "1",
        StringComparison.Ordinal);
    ...
    /// HYPER5_TEXTURE_DUMP_DIR is set, so upload-time content can be
    ...
    private static void DumpTextureSourceIfRequested(
        ...
        byte[] source)
    {
        var directory = Environment.GetEnvironmentVariable("HYPER5_TEXTURE_DUMP_DIR");
        ...
        {
            // A bad HYPER5_TEXTURE_DUMP_DIR (permissions, invalid path)
        ...
        Gen5ShaderProgram program)
    {
        if (shader.Payload.Length == 0 ||
            !string.Equals(
                Environment.GetEnvironmentVariable("HYPER5_DUMP_SPIRV"),
                "1",
                StringComparison.Ordinal))
        {
            return;
    ...
    private static void TraceCreateShader(ulong destinationAddress, ulong headerAddress, ulong codeAddress, string detail)
    {
        ...
        if (isOk &&
            (!string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_AGC"), "1", StringComparison.Ordinal) ||
